...
### Viewport Height
NEVER use `h-screen` for full-height sections. ALWAYS use `min-h-[100dvh]` to prevent layout jumping on mobile (iOS Safari address bar).

### Grid Over Flex Math
NEVER use complex flexbox percentage math (`w-[calc(33%-1rem)]`). ALWAYS use CSS Grid (`grid grid-cols-1 md:grid-cols-3 gap-6`).

### Container Max Width
Contain page layouts using `max-w-[1400px] mx-auto` or `max-w-7xl`.

### Anti-Center Bias
When DESIGN_VARIANCE > 4, centered Hero/H1 sections are discouraged. Prefer split screen (50/50), left-aligned content/right-aligned asset, or asymmetric whitespace.

### Mobile Override Rule
For DESIGN_VARIANCE 4-10, any asymmetric layout above `md:` MUST fall back to single-column (`w-full`, `px-4`, `py-8`) on viewports < 768px.
