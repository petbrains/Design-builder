...
Anti-patterns to flag per designer perspective, plus general motion mistakes. Pair with [`audit-checklist.md`](audit-checklist.md) for systematic review.

> **Note:** The "no bounce/elastic easing" rule lives in [`../motion-design.md`](../motion-design.md) (hub) to avoid duplication — check there first.
...
- **Animations over 300ms** (productivity context) — UI animations should be under 300ms; 180ms feels more responsive than 400ms
...
- **No animation at all** — Instant state changes feel broken to modern users (see [`motion-gaps.md`](motion-gaps.md))
