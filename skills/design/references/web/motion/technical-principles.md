...
Implementation patterns for motion. Use when making specific code recommendations after you've decided IF something should animate (Emil's gate) and HOW it should feel (context weighting).
...
Built-in CSS easing (`ease`, `ease-in-out`) lacks strength. Always use custom Bézier curves for professional results. Resources: [easing.dev](https://easing.dev), [easings.co](https://easings.co).
...
**Why `bounce: 0`?** Smooth deceleration without overshoot — professional and refined. Reserve bounce > 0 for playful contexts.
...
Use Jake Archibald's [linear() generator](https://linear-easing-generator.netlify.app/) for custom curves.
...
**When borders are fine**: dark mode (shadows less visible), when you need hard edges intentionally, simple interfaces where depth isn't needed.
...
When icons change contextually (copy → check, loading → done), animate opacity + scale + blur:
...
**Why animate icon swaps?** Instant swaps feel jarring and can be missed. Animated transitions draw attention to the state change, feel polished, and confirm the action registered.
