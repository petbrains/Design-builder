...
Emil Kowalski is a Design Engineer at Linear (previously Vercel). Creator of [Sonner](https://sonner.emilkowal.ski), [Vaul](https://vaul.emilkowal.ski), and the [Animations on the Web](https://animations.dev) course. His approach emphasizes **restraint, speed, and purposeful motion**.
...
Subtle scale-down on press for immediate feedback:
```css
button:active {
...
Animating from `scale(0)` creates unnatural motion. Start from `scale(0.9)` or higher:
```jsx
// BAD
initial={{ scale: 0 }}
...
First tooltip: delay + animation. Subsequent tooltips in same group: instant:
```css
...
Built-in `ease` / `ease-in-out` lack strength. Use custom Bézier curves:
