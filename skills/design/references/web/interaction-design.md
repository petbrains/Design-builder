...
## Component Interaction Patterns (from Emil Kowalski)
...
Popovers should scale from their trigger, not center. Exception: modals keep center origin.
...
First tooltip delays before appearing. Once one is open, subsequent tooltips open instantly with no animation.

### Use blur to mask imperfect transitions
When a crossfade feels off, add subtle `filter: blur(2px)` during transition. Blur bridges the gap between two states. Keep under 20px (expensive in Safari).

### Gesture patterns
- **Momentum dismissal**: Calculate velocity (`distance / time`). If > 0.11, dismiss regardless of distance.
- **Damping at boundaries**: Apply increasing friction past natural boundary, not hard stops.
- **Pointer capture**: Set element to capture all pointer events once drag starts.
