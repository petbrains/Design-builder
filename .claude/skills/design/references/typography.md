...
## Recommended Font Stacks by Context

For technical/dashboard UIs where serif is inappropriate:
- `Geist` + `Geist Mono` — Clean, modern, Vercel-inspired
- `Satoshi` + `JetBrains Mono` — Geometric, slightly warmer

For creative/editorial contexts:
- Browse Pangram Pangram, Future Fonts, ABC Dinamo for distinctive options
- `Cabinet Grotesk` — Strong geometric character for headlines

**Note**: These are starting points, not defaults. Always run the font selection procedure from the main skill before committing to any font.
